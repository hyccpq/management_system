export const INCREMENT = 'INCREMENT';
export type INCREMENT = typeof INCREMENT;

export const DECREMENT = 'DECREMENT';
export type DECREMENT = typeof DECREMENT;

export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export type LOGIN_SUCCESS = typeof LOGIN_SUCCESS;

export const LOGIN_FAIL = 'LOGIN_FAIL';
export type LOGIN_FAIL = typeof LOGIN_FAIL;

export const LOGIN_ERROR = 'LOGIN_ERROR';
export type LOGIN_ERROR = typeof LOGIN_ERROR;

export const LOADING_STATE = 'LOADING_STATE';
export type LOADING_STATE = typeof LOADING_STATE;


//Statistic
export const STATISTIC_SUCCESS = 'STATISTIC_SUCCESS';
export type STATISTIC_SUCCESS = typeof STATISTIC_SUCCESS;

//UserInfo
export const USERINFO_SUCCESS = 'USERINFO_SUCCESS';
export type USERINFO_SUCCESS = typeof USERINFO_SUCCESS;

//ProductList
export const PRODUCT_LIST_SUCCESS = 'PRODUCT_LIST_SUCCESS';
export type PRODUCT_LIST_SUCCESS = typeof PRODUCT_LIST_SUCCESS;

//ProductShow
export const PRODUCT_SHOW_SUCCESS = 'PRODUCT_SHOW_SUCCESS';
export type PRODUCT_SHOW_SUCCESS = typeof PRODUCT_SHOW_SUCCESS;

//ProductClass
export const PRODUCT_CLASS_SUCCESS = 'PRODUCT_CLASS_SUCCESS';
export type PRODUCT_CLASS_SUCCESS = typeof PRODUCT_CLASS_SUCCESS;